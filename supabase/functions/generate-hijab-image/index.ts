...
function getColorName(hex: string): string {
  const colorMap: Record<string, string> = {
    "#1e3a5f": "navy blue", "#1a1a1a": "black", "#f5f5f0": "off-white",
    "#d4b896": "beige", "#4a6741": "olive green", "#6b4226": "brown",
    "#7a7d80": "gray", "#722f37": "burgundy", "#2e8b8b": "teal",
    "#c97b84": "dusty pink", "#6ca0dc": "sky blue", "#cc5500": "burnt orange",
  };
...
function getSkinToneName(hex: string): string {
  const toneMap: Record<string, string> = {
    "#fde7d0": "very fair skin", "#f5d0a9": "fair skin",
    "#e8b88a": "light wheat skin", "#d4a06a": "wheat skin",
    "#c08c5a": "warm tan skin", "#a87844": "olive skin",
    "#8d6535": "medium brown skin", "#6b4423": "dark brown skin",
  };
...
function getSuggestedHijabColors(outfitColor: string): { name: string; hex: string }[] {
  const suggestions: Record<string, { name: string; hex: string }[]> = {
    "#1e3a5f": [{ name: "burnt orange", hex: "#cc5500" }, { name: "dusty rose", hex: "#c97b84" }],
    "#1a1a1a": [{ name: "emerald green", hex: "#2e8b57" }, { name: "burgundy red", hex: "#722f37" }],
    "#f5f5f0": [{ name: "navy blue", hex: "#1e3a5f" }, { name: "dusty mauve", hex: "#b0879b" }],
    "#d4b896": [{ name: "teal", hex: "#2e8b8b" }, { name: "chocolate brown", hex: "#6b4226" }],
    "#4a6741": [{ name: "mustard yellow", hex: "#c7a93c" }, { name: "cream", hex: "#f5f0e1" }],
    "#6b4226": [{ name: "sky blue", hex: "#6ca0dc" }, { name: "sage green", hex: "#9caf88" }],
    "#7a7d80": [{ name: "blush pink", hex: "#de98a0" }, { name: "royal blue", hex: "#4169e1" }],
    "#722f37": [{ name: "gold", hex: "#c7a93c" }, { name: "cream white", hex: "#f5f0e1" }],
    "#2e8b8b": [{ name: "burnt orange", hex: "#cc5500" }, { name: "coral", hex: "#e07060" }],
    "#c97b84": [{ name: "olive green", hex: "#4a6741" }, { name: "charcoal", hex: "#36454f" }],
    "#6ca0dc": [{ name: "peach", hex: "#f0a080" }, { name: "tan", hex: "#c8a882" }],
    "#cc5500": [{ name: "navy blue", hex: "#1e3a5f" }, { name: "forest green", hex: "#355e3b" }],
  };
  return suggestions[outfitColor] || [
    { name: "cream white", hex: "#f5f0e1" }, { name: "dusty rose", hex: "#c97b84" },
  ];
}
...
serve(async (req) => {
  ...
  try {
    ...
    const imagePromises = hijabSuggestions.map(async (hijab) => {
      ...
      if (userPhoto) {
        // Edit mode: overlay hijab on user's photo
        messages = [
          {
            role: "user",
            content: [
      ...
      return {
        ...
        description: userPhoto
          ? `حجاب ${hijab.name} على صورتك مع ${colorName}`
          : `تنسيق حجاب ${hijab.name} مع ${colorName}`,
      };
    });
